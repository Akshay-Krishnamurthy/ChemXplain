...
def run_reinvent4(smiles,exe,prior,num_smiles=200,temperature=1.2,
    ...
    try:
        ...
        # Preserve stereochemistry so tokens like [C@@H] match the prior vocab
        clean=Chem.MolToSmiles(mol,isomericSmiles=True).strip()
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with c2:
        prop2=st.selectbox("⚖️ Secondary property (trade-off):",fav+[p for p in all_props if p not in fav],index=3)
...
""")
    ...
    if st.session_state.get("optimization_run") and "res_df" in st.session_state:
        ...
        st.markdown("---")
        st.success(f"✅ **{len(res)}** molecules generated · **{len(imp)}** improved {prop1}")
        ...
        if not imp.empty:
            ...
            legends=[f"[{row['Method']}]\n{prop1}:{row['New_Score']:.3f} Δ{row['Delta']:+.3f}\nSA:{row['SA_Score']:.1f}" for _,row in top5.iterrows()]
...
New {prop1}: {best['New_Score']:.3f}  (Δ = {best['Delta']:+.3f})
...
PART 2 — PHARMACOLOGICAL RATIONALE: Explain WHY this change improved {prop1}.
