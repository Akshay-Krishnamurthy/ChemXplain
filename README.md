...
$$W_i = f_P(G_{\text{baseline}}) - f_P(G \setminus \{v_i\})$$

Where:
- $f_P$ is the ADMET-AI graph neural network predicting property $P$
- $G_{\text{baseline}}$ is the original molecular graph
- $G \setminus \{v_i\}$ is the graph with atom $i$ replaced by a wildcard node (atomic number 0)
- $W_i$ is the **directional causal weight** of atom $i$

**Interpretation:**
- $W_i > 0$ → removing atom $i$ **lowers** the score → atom is a **liability driver**
- $W_i < 0$ → removing atom $i$ **raises** the score → atom is a **protective suppressor**

Normalized absolute weights are used for visualization:

$$\hat{W}_i = \frac{|W_i| - \min_j|W_j|}{\max_j|W_j| - \min_j|W_j|}$$

This normalized $\hat{W}_i \in [0, 1]$ drives the 2D Sharp Map color gradient and 3D sphere sizing.
...
$$B_k = \left| f_P(G_{\text{baseline}}) - f_P(G \setminus \{e_k\}) \right|$$
...
All atoms in a ring system $\mathcal{R}_j$ are simultaneously masked:

$$M_j = \left| f_P(G_{\text{baseline}}) - f_P\left(G \setminus \mathcal{R}_j\right) \right|$$
...
Two properties $P_1$ and $P_2$ are simultaneously ablated per atom:

$$\mathbf{v}_i = (W_i^{P_1},\ W_i^{P_2})$$

Atoms plotted in the **bottom-left quadrant** satisfy $W_i^{P_1} < 0$ AND $W_i^{P_2} < 0$, meaning their removal suppresses **both** liabilities simultaneously — these are the highest-priority medicinal chemistry targets.
...
$$\text{Mutants} = \{ G' \mid G' = G[v_t \leftarrow f_c],\ f_c \in \mathcal{F}_{\text{ChEMBL}},\ \text{context}(f_c, G, v_t, r=1) \}$$
...
$$SA\_Score = \text{fragment\_score} - \text{complexity\_penalty}$$
...
$$AD\_score = \max_{r \in \mathcal{R}} T\left(FP_{\text{query}},\ FP_r\right)$$

- $AD > 0.3$: High confidence (scaffold is pharmaceutical-like)
- $0.15 < AD \leq 0.3$: Moderate confidence
- $AD \leq 0.15$: Alien chemotype — predictions may be unreliable
